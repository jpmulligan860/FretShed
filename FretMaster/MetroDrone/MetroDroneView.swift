                ...
                ForEach(Array(vm.accents.enumerated()), id: \.offset) { index, accent in
                    Button { vm.cycleAccent(at: index) } label: {
                        accentLabel(accent)
                            ...
                            .background(accentButtonBackground(accent), in: RoundedRectangle(cornerRadius: 8))
                    }
                    .buttonStyle(.plain)
                }
            }
    ...
    private var playButtons: some View {
        HStack(spacing: 16) {
            Button(action: vm.toggleMetronome) {
                Label(
                    vm.isMetronomePlaying ? "Stop" : "Metronome",
                    systemImage: vm.isMetronomePlaying ? "stop.fill" : "play.fill"
                )
                .font(.headline)
                .frame(maxWidth: .infinity, minHeight: 50)
                .foregroundStyle(.white)
                .background(vm.isMetronomePlaying ? .red : .indigo, in: RoundedRectangle(cornerRadius: 14))
            }
            .buttonStyle(.plain)

            Button(action: vm.toggleDrone) {
                Label(
                    vm.isDronePlaying ? "Stop" : "Drone",
                    systemImage: vm.isDronePlaying ? "stop.fill" : "play.fill"
                )
                .font(.headline)
                .frame(maxWidth: .infinity, minHeight: 50)
                .foregroundStyle(.white)
                .background(vm.isDronePlaying ? .red : .teal, in: RoundedRectangle(cornerRadius: 14))
            }
            .buttonStyle(.plain)
        }
    }
        ...
        DisclosureGroup("Speed Trainer", isExpanded: $showSpeedTrainer) {
            VStack(spacing: 16) {
                ...
                Button(action: vm.toggleSpeedTrainer) {
                    ...
                    .foregroundStyle(.white)
                    .background(vm.isSpeedTrainerActive ? .red : .orange, in: RoundedRectangle(cornerRadius: 10))
                }
                .buttonStyle(.plain)
            }
            .padding(.top, 8)
                    ...
                    ForEach(MusicalNote.allCases) { note in
                        Button {
                            ...
                            Text(note.sharpName)
                                ...
                                .background(
                                    vm.droneKey == note ? Color.teal : Color.gray.opacity(0.15),
