// PitchDetector.swift
// FretMaster — Audio Layer (Phase 5)
//
...
private let logger = Logger(subsystem: "com.jpm.fretmaster", category: "PitchDetector")
...
private func makeTapClosure(
    ...
    return { buffer, _ in
        ...
        var sum: Float = 0
        vDSP_svesq(state.analysisBuffer, 1, &sum, vDSP_Length(windowSize))
        let rms = sqrt(sum / Float(windowSize))
        // Map RMS to 0–1 using dB scale: -50 dB → 0.0, 0 dB → 1.0
        let rmsLevel = max(0, min(1, (20 * log10(max(rms, 1e-10)) + 50) / 50))

        // Adaptive noise gate: tracks noise floor, sets threshold dynamically
        if rms < state.noiseFloor {
            state.noiseFloor = rms
        } else {
            state.noiseFloor += (rms - state.noiseFloor) * 0.0005
        }
        let gateThreshold = max(state.noiseFloor * 4.0, 0.002)
