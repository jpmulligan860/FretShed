...
| 2026-03-18 | Spacing Gate & Smart Review system complete (SG.1–SG.6). Spaced repetition mastery: 3 checkpoints across 5+ days, single-step regression, completed gate permanent. Always-on review block (30% of session). Fluency focus mode rotation (4 modes). Heatmap: proficient=gold, mastered=green. isMastered no longer requires 15 attempts (spacing gate replaces attempt gate). Default session length fixed to 20. All pre-existing test failures resolved. Test count 448 (0 failures). Next task: Phase 4 (Monetization). | ROADMAP_STRATEGY.md, CLAUDE_STRATEGY.md | 🔲 Pending |
