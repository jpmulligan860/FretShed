...
public struct SettingsView: View {
    ...
    private func audioSection(settings: UserSettings) -> some View {
        Section {
            ...
            Toggle("Correct Note Sound", isOn: Binding(
                get: { settings.correctSoundEnabled },
                set: { settings.correctSoundEnabled = $0; save(settings) }
            ))
            ...
            Toggle("Incorrect Note Sound", isOn: Binding(
                get: { settings.incorrectSoundEnabled },
                set: { settings.incorrectSoundEnabled = $0; save(settings) }
            ))
            ...
            Text("Higher confidence reduces false notes. Longer hold duration prevents fleeting detections from registering.")
        }
