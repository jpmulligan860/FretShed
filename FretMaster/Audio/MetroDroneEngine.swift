...
private let logger = Logger(subsystem: "com.jpm.fretmaster", category: "MetroDrone")
...
final class MetroDroneEngine {
    ...
    func startMetronome(
        ...
        metronomeTask = Task { [weak self] in
            ...
            // interval before the first click to avoid doubling up with the
            // old task's final beat click.
            if delayFirstBeat {
    ...
    func updateMetronomeTempo(bpm: Double, timeSignature: TimeSignature, accents: [BeatAccent], volume: Float) {
        guard isMetronomePlaying else { return }
        startMetronome(bpm: bpm, timeSignature: timeSignature, accents: accents, volume: volume, delayFirstBeat: true)
    }
