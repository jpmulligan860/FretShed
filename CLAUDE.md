...
| 2026-03-18 | Spacing Gate & Smart Review system (SG.1–SG.9). Spaced repetition mastery: 3 checkpoints across 5+ days, single-step regression, completed gate permanent. Always-on review block (30%). Fluency rotation (4 modes). Heatmap: proficient=gold, mastered=green. Phase Roadmap on Journey tab. Woodshop font sweep (50+ fixes). Terminology consistency (mastered→proficient in phase messages). Default session length=20. Test count 448 (0 failures). Next: Phase 4. | ROADMAP_STRATEGY.md, CLAUDE_STRATEGY.md | 🔲 Pending |
