...
public struct SessionSetupView: View {
            ...
            HStack(spacing: 8) {
                ...
                ForEach([6, 5, 4, 3, 2, 1], id: \.self) { string in
                    ...
                    Button {
                        ...
                        .padding(.vertical, 8)
                        .background(
                            isSelected ? Color.indigo : Color(.secondarySystemGroupedBackground),
                            in: Capsule()
                        )
                    ...
                    ForEach(MusicalNote.allCases, id: \.self) { note in
                        Button {
                            ...
                            Text(note.displayName(format: .sharps))
                                ...
                                .padding(.vertical, 8)
                                .background(
                                    selectedNote == note ? Color.indigo : Color(.secondarySystemGroupedBackground),
                                    in: Capsule()
                                )
            ...
            HStack(spacing: 16) {
                Button {
                    ...
                    Image(systemName: "minus.circle.fill")
                        .font(.title2)
                        .foregroundStyle(sessionLength > 5 ? .indigo : .secondary)
                }
                .buttonStyle(.plain)
                ...
                Button {
                    ...
                    Image(systemName: "plus.circle.fill")
                        .font(.title2)
                        .foregroundStyle(sessionLength < 100 ? .indigo : .secondary)
                }
                .buttonStyle(.plain)
                                ...
                                if fret > 0 {
                                    ...
                                    Circle()
                                        .fill(Self.isMarkerFret(fret)
                                              ? (isSelected ? Color.white.opacity(0.7) : Color.indigo.opacity(0.5))
                                              : Color.clear)
                        ...
                        ForEach(MusicalNote.allCases, id: \.self) { note in
                            Button {
                                ...
                                Text(note.sharpName)
                                    ...
                                    .padding(.vertical, 7)
                                    .background(
                                        progressionKey == note ? Color.indigo : Color(.secondarySystemGroupedBackground),
                                        in: Capsule()
                                    )
            ...
            LazyVGrid(columns: [.init(.flexible()), .init(.flexible())], spacing: 8) {
                ForEach(Array(ChordProgression.presets.enumerated()), id: \.offset) { idx, preset in
                    Button {
                        ...
                        .padding(.vertical, 8)
                        .background(
                            selectedPresetIndex == idx ? Color.indigo : Color(.secondarySystemGroupedBackground),
                            in: RoundedRectangle(cornerRadius: 10)
                        )
                ...
                Button {
                    ...
                    .padding(.vertical, 8)
                    .background(
                        selectedPresetIndex == nil ? Color.indigo : Color(.secondarySystemGroupedBackground),
                        in: RoundedRectangle(cornerRadius: 10)
                    )
    ...
    private func startSession() {
        ...
        Task {
            ...
            await MainActor.run { activeQuizVM = vm }
        }
    }
