...
> **Product name decided**: FretShed: Smart Practice · Bundle ID: `com.jpm.fretshed` (locked in App Store Connect)
...
| SP.8 | Phase Resequencing & Insight Accuracy — Phase 2↔3 swap (Expansion before Connection), fret range 0→12, all 6 strings required for Phase 1, Phase 2 per-string tracking, diagnostic/confirmation session builders, targetStrings enforcement in QuizViewModel, insight factual accuracy fixes (7 false-claim bugs + contradiction detection test infra), Next Up session consistency, dev button visibility fix, merged InputLevelBar into micListeningView, phase copy fix ("the Expansion phase"), stale Shed phase refresh, weakest-string calculation (count untried cells), removed all free-tier string/fret restrictions, chromaticFragment GroupType for accidentals-only groups, TunerView mic alert cross-tab fix. 425 tests (27 new). | ✅ |

---

## Spacing Gate & Smart Review

**Spaced repetition mastery system + always-on review block + Fluency focus mode rotation.**

| # | Task | Status |
|---|---|---|
| SG.1 | Spacing Gate Data Model — 3 checkpoint dates on MasteryScore (SwiftData auto-migration), `hasCompletedSpacingGate`, `regressSpacingCheckpoint()`, BackupPayload v2 backward compat | ✅ |
| SG.2 | Mastery Promotion Logic — `isMastered` requires score ≥ 0.75 + all 3 checkpoints (attempt minimum removed). `tryAdvanceCheckpoint()` with calendar-day gap enforcement (1 day CP2, 3 days CP3) and `checkpointMinAttempts = 3`. Heatmap colors swapped: proficient = gold, mastered = green | ✅ |
| SG.3 | Always-On Review Block — 30% of session (min 3, max 10) from completed strings every Smart Practice session. Prioritizes active checkpoint progression + low effectiveScore. Warmup intro card only after 1+ day away. "Quick Review" label in quiz UI (portrait + landscape) | ✅ |
...
| SG.8 | Woodshop Font Sweep — Replaced ~50 system font references with DesignSystem.Typography tokens across 14 files. 10 new tokens added (accentBody, accentLarge, bodyText, dataMicro, dataTiny, dataPip, dataChip, promptLabel, feedbackLabel, compactNote). Only SF Symbol icon sizing remains as .system() | ✅ |
| SG.9 | Terminology Consistency — "mastered"→"proficient" in all phase messages (PhaseInsightLibrary, SmartPracticeEngine). Phase 2 label "Sharps & Flats"→"All Notes" for accurate count. Stale "15+ attempts" descriptions updated for spacing gate. Heatmap info corrected (proficient=gold, mastered=green). Phase indicator moved below rig card on Shed page | ✅ |
...
| 4.11 | Test purchases with sandbox account — StoreKit local config, sandbox purchase flow verified, isPremium flips, locks disappear, persists across relaunch. Free-tier gating verified at all layers (UI, engine, quiz). | 1 hr | ✅ |
