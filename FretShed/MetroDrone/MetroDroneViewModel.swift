...
private let logger = Logger(subsystem: "com.jpm.fretshed", category: "MetroDroneVM")

// MARK: - Note Subdivision

enum NoteSubdivision: String, CaseIterable, Sendable {
    case quarter   = "quarter"
    case eighth    = "eighth"
    case triplet   = "triplet"
    case sixteenth = "sixteenth"

    /// Number of evenly-spaced clicks per beat (including the beat itself).
    var count: Int {
        switch self {
...
final class MetroDroneViewModel {
    ...
    func startSpeedTrainer() {
        ...
        if hasCountIn {
            ...
            // Already playing — update BPM seamlessly
            engine.updateMetronomeBPM(bpm)
            if isDronePlaying { engine.updateDroneLFORate(bpm: bpm) }
        }
    }
    ...
    func setTimeSignature(_ ts: TimeSignature) {
        ...
        if engine.isMetronomePlaying {
            engine.updateMetronomeTimeSignature(ts, accents: accents)
        }
    }
    ...
    func setSubdivision(_ sub: NoteSubdivision) {
        ...
        if engine.isMetronomePlaying {
            engine.updateMetronomeSubdivision(sub)
        }
    }
    ...
    func cycleAccent(at index: Int) {
        ...
        if engine.isMetronomePlaying {
            engine.updateMetronomeAccents(accents)
        }
    }
        ...
        if isCountingIn {
            ...
            if countInBeatCount >= timeSignature.beats {
                ...
                if countInBarCount >= countInBars {
                    isCountingIn = false
                    countInBarsRemaining = 0
                    // Count-in used quarter notes — switch to selected subdivision
                    if subdivision != .quarter {
                        engine.updateMetronomeSubdivision(subdivision)
                    }
                }
            }
            return
    ...
    private enum Keys {
        ...
        static let subdivision = "metroDrone.subdivision"
    }
    ...
    private func loadSettings() {
        ...
        if let raw = d.string(forKey: Keys.subdivision),
           let sub = NoteSubdivision(rawValue: raw) {
            subdivision = sub
        }
