...
// TunerView.swift
// FretMaster — Presentation Layer (Phase 5)
                            ...
                            Group {
                                ...
                                case .needle:
                                    NeedleDisplay(cents: detector.centsDeviation,
                                        ...
                                        .padding(.top, 12)
                                case .strobe:
                                    StrobeDisplay(cents: detector.centsDeviation,
                                                  isActive: detector.detectedNote != nil)
                                        .padding(.top, 12)
                                }
                            }
    ...
    private var controls: some View {
        VStack(spacing: 16) {
            Picker("Display", selection: Binding(
                get: { displayStyle },
                set: { newStyle in
                    displayStyleRaw = newStyle.rawValue
                    if let s = settings {
                        s.tunerDisplayStyle = newStyle
                        try? container.settingsRepository.saveSettings(s)
                    }
                }
            )) {
                Label("Needle", systemImage: "gauge.medium").tag(TunerDisplayStyle.needle)
                Label("Strobe", systemImage: "waveform").tag(TunerDisplayStyle.strobe)
            }
            .pickerStyle(.segmented)
            .padding(.horizontal, 40)

            HStack(spacing: 8) {
                ForEach(ReferenceA.allCases, id: \.rawValue) { ref in
                    Button {
                        referenceAHz = ref.rawValue
                        detector.referenceA = ref.hertz
                        if let s = settings {
                            s.referenceAHz = ref.rawValue
                            try? container.settingsRepository.saveSettings(s)
                        }
                    } label: {
                        Text(ref.label)
                            ...
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 10)
                            .background(referenceAHz == ref.rawValue
                                        ? Color.indigo
                                        : Color(.secondarySystemGroupedBackground),
                                        in: RoundedRectangle(cornerRadius: 10))
                            .foregroundStyle(referenceAHz == ref.rawValue ? .white : .primary)
                    }
                    .buttonStyle(.plain)
                }
            }
...
private struct NeedleDisplay: View {
    ...
    var body: some View {
        ZStack {
            DialArc()
                .stroke(Color(.tertiarySystemGroupedBackground), lineWidth: 4)
